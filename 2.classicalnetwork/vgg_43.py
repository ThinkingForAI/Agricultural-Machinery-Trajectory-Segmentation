# -*- coding: utf-8 -*-
"""
Created on Thu Feb  1 07:39:16 2024
...
def wrap(path):
    ...
    for name in excel_names:
        ...
        X+=data.drop([43],axis=1).astype(float).values.tolist()
        y+=data.iloc[:,43].tolist()
    return X,y
#采用划分处理后的数据
X_test,y_test=wrap('../classicalnetwork/vgg_data/test_43')
X_valid,y_valid=wrap('../classicalnetwork/vgg_data/valid_43')
X_train,y_train=wrap('../classicalnetwork/vgg_data/train_43')
...
class mydataset(Dataset):
    ...
    def __init__(self,data,label,mode='valid'):
        ...
        #(n,43)->(n,1,10,10)
        #通道增加
        data=np.array(data)[:,:36].reshape(np.array(data)[:,:36].shape[0],1,6,6)
