...
train=CIFAR100('../Mixer',train=True,download=True)
test=CIFAR100('../Mixer',train=False,download=True)
...
trainloader=DataLoader(traindata,batch_size=512,shuffle=True,generator=torch.Generator(device='cuda'))
testloader=DataLoader(testdata,batch_size=512,shuffle=False,generator=torch.Generator(device='cuda'))
...
mixer.load_state_dict(torch.load('../MixerReplication/mixer.params'))

with torch.device('cuda'):
    loss_fn=CrossEntropyLoss()#Softmax included
    optimizer=SGD(mixer.parameters(),lr=0.1)
    epoches=100
    scheduler=lr_scheduler.LinearLR(optimizer,start_factor=1,end_factor=0.001,total_iters=epoches*len(trainloader))
    losslist=[]
    for i in range(epoches):
        ...
        for idx,(X,y) in enumerate(trainloader):
            ...
            # clip_grad_norm_(mixer.parameters(), max_norm=10,norm_type=1)#gradient-clipping  if ratio>1,then scale return total_norm
            optimizer.step()
            scheduler.step()
            optimizer.zero_grad()
            avgloss+=loss.item()
